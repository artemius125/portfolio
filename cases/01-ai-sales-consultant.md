...
Я сделал это в Клерк.Ру, бухгалтерском портале, с января по июнь 2026 года. Работал там единственным AI-разработчиком. Это чат-консультант для B2B-медиапортала компании: он отвечает посетителям по каталогу рекламных услуг и передает заявку менеджеру.
...
У портала профессионального сообщества был раздел с рекламными услугами: статьи, баннеры, рассылки, посты в соцсетях, видео, ведение блога компании. Часть прайса лежала на статической странице, часть в тарифном API портала. Всё остальное уходило в почту менеджеру, и ответ приходил через часы. Отдел продаж хотел консультировать посетителей в чате сразу на лендинге и честно передавать сложные вопросы человеку. Ещё отдел продаж хотел сам менять прайс и поведение бота, без релизов и без разработчика. Бот не должен был выдумывать цены: цена в диалоге приравнивалась к оферте.

## Что я сделал

- Собрал сервис на FastAPI со стримингом ответов через SSE и написал встраиваемый JS-виджет для лендинга.
- Перенес прайс и системный промпт из JSON и кода в PostgreSQL: сделал в админке CRUD для прайса, CSV-импорт и версии промпта с откатом и журналом аудита.
- Подключил LLM через OpenAI-совместимый шлюз с запасной моделью на случай сбоя основной, без RAG и без векторной базы: каталог услуг целиком помещается в системный промпт.
- Сделал протокол текстовых маркеров вместо function calling, чтобы бот мог показывать кнопки действий на любой модели.
- Настроил проверку сессии портала через прокси cookie с кэшем на 60 секунд, чтобы не дергать апстрим на каждое сообщение чата.
- Добавил аналитику: воронку от диалога до заявки, атрибуцию услуг к сообщениям бота, A/B тесты приветствия.
- Упаковал сервис в Docker и развернул в Kubernetes с двумя репликами, RollingUpdate и пробами готовности.

## Что было сложно

Часть LLM-моделей класса reasoning возвращала content: null в дельте: весь текст уходил в поле reasoning, и наружу приходил пустой ответ при HTTP 200. Я заметил это по логам с пустыми сообщениями и добавил проверку, пришел ли непустой текст, с переходом на следующую модель в цепочке. Дефолтной моделью осталась обычная, не-reasoning.

Ещё один баг: генератор SSE живет дольше, чем request-scope, и держал бы соединение с БД всё время генерации ответа. Я разделил работу с базой на две сессии: все чтения (промпт, прайс, история) идут в короткой сессии до старта стрима, а сохранение ответа ассистента в новой сессии внутри генератора.

Нашел IDOR: эндпоинт истории диалога по UUID отдавал чужую переписку без проверки прав. Он не был нужен ни виджету, ни админке, я его удалил и оставил доступ к истории только через админку. Также закрыл открытый эндпоинт трекера событий: добавил whitelist типов событий, проверку существования сессии, rate-limit по IP и валидацию UUID, чтобы кривой ввод отдавал 422, а не 500.

## Результат

Сервис работает в проде: две реплики, RollingUpdate, три вида проб, восемь примененных миграций схемы. Отдел продаж сам управляет прайсом, промптом, квотами и текстами виджета, без участия разработчика. Есть аналитика воронки и атрибуция услуг, экспорт диалогов в JSON и CSV. Продуктовых метрик вроде конверсии или числа диалогов компания не фиксировала.

## Моя роль

Я сделал бэкенд, админку, интеграцию с LLM-шлюзом и аналитику, плюс всю деплойную обвязку: Dockerfile, Helm values, пробы, логирование. Продуктовые решения, например тексты бота и правила передачи в поддержку, принимал заказчик. Конвенции кластера, домены и CI/CD-переменные задавала платформенная команда заказчика.
