...
Кейс сделан в компании Клерк.Ру, бухгалтерском портале с около 7 млн визитов в месяц, с января по июнь 2026 года. Я работал там единственным AI-разработчиком. Задача: чат-сервис, который в диалоге подбирает пользователю учебный трек из каталога онлайн-школы и не может выдумать курс, цену или ссылку.
...
Онлайн-школа при бухгалтерском портале держала каталог примерно из ста программ: длинные курсы повышения квалификации с выдачей документа и короткие онлайн-курсы в основном по подписке. Пользователь на сайте обычно не знал, что ему нужно, и формулировал запрос свободно, например «хочу разобраться в управленческом учёте». Агент уже существовал, но был написан на TypeScript и жил внутри монолитного Next.js-приложения. Сервис не имел своего логина: доступ проверялся по профилю пользователя на портале, с разными правами для подписки, триала и админа. В каталоге портала не хватало полей, важных для подбора: уровня программы, предварительных требований, смежных тем. В кластере не было планировщика задач, поэтому обновление каталога нужно было встроить в сам сервис.

## Что я сделал

- Перенёс агента с TypeScript внутри монолита на отдельный Python-бэкенд на FastAPI. Так сервис получил свой деплой и масштабирование без зависимости от фронтенда портала.
- Построил детерминированный движок подбора без LLM: свой field-weighted BM25 по каталогу с весами полей и лёгким стеммером для русских словоформ. Это дало предсказуемый и тестируемый поиск основного курса без обращения к модели.
- Добавил отдельную стадию выбора: один дешёвый вызов LLM смотрит на десяток кандидатов от BM25 и выбирает основной курс с учётом заявленного уровня и предпочтений, которые регулярками не поймать.
- Сделал добор дополняющих онлайн-курсов детерминированным: он идёт по редакционным смежным темам основного курса и отбрасывает кандидатов, чьё содержание уже покрыто.
- Написал валидатор, который подставляет названия, ссылки, цены и статус подписки только из каталога. Модель в ответах передаёт только id курса, а не сами данные.
- Сделал потоковый фильтр ссылок по allow-list доменов поверх ответа модели: он работает чанк за чанком, чтобы выдуманная ссылка не появилась в интерфейсе даже на короткое время.
- Настроил фоновую синхронизацию каталога раз в сутки под postgres advisory-lock и LLM-обогащение полей, которых нет в API портала: уровень, компетенции, смежные темы.

## Что было сложно

Преемственность трека между ходами диалога. Когда пользователь просил «добавь про налоги», агент должен был дорабатывать уже собранный трек, а не собирать новый с нуля. Я восстанавливал прошлый результат инструмента из истории сообщений: искал последний успешный вызов и доставал из него основной курс и список онлайн-курсов. Отдельно закрепил «пин» основного курса на случай, если пользователь уже на нём учится.

Модель иногда соглашалась с цифрами, которые пользователь называл «по памяти», например «там же 120 часов?», и придумывала ссылку на курс, если под рукой её не было. Я стал класть в служебную сводку истории правдивые факты по курсам трека: ссылку и количество часов. Тогда модели есть с чем сверяться, а не только слова пользователя.

Была и чисто техническая проблема: request-scoped сессия БД закрывалась сразу после ответа, а генератор SSE жил дольше и в конце писал в базу лог рекомендации и расход токенов. Пришлось открывать отдельную сессию на время стрима. А ручная калибровка скоринга регулярно приводила к регрессиям: несколько правок и результат мог разъехаться целиком. Я завёл golden-кейсы с ожидаемым основным курсом и eval-харнесс, который считает метрики и падает при просадке порога.

## Результат

Eval-харнесс на размеченном наборе запросов падает, если top-1 hit-rate опускается ниже 0.8 или хотя бы один внедоменный запрос перестаёт честно отклоняться. Весь контур подбора покрыт тестами без БД и без сети: golden-сценарии, юнит-тесты стемминга, ранжирования и фильтра по подписке. Трек всегда состоит из основного курса и от 3 до 7 дополняющих онлайн-курсов без дублирования содержания. Продуктовых метрик, таких как конверсия в оплату, компания не собирала.

## Моя роль

Я перенёс агента с TypeScript внутри монолита на самостоятельный Python-бэкенд: движок подбора, диалоговый слой, фильтр ссылок, схему БД, синхронизацию каталога, админку, тесты и деплой. Продуктовую логику (структуру трека, разделение типов курсов, смежные темы) и содержимое каталога определяла компания. Часть архитектурных решений, например разделение труда между движком и моделью, я вырабатывал вместе с заказчиком по результатам эвалов.
