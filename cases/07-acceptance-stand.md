...
Кейс из компании maracuya ai, стартапа с платформой ИИ-агентов для работы с документами. Я работал там LLM-инженером с июня по сентябрь 2026 года. С 11 по 28 августа 2026 я построил приёмочный стенд, который проверяет не ход агента, а сам файл, полученный пользователем.
...
Платформа maracuya ai получает от пользователя запрос и обычно файл, а инструмент отдаёт готовый документ: договор с правками, смету, презентацию. Прежняя приёмка проверяла не результат, а ход агента: отработал ли инструмент, завершился ли шаг, выглядит ли ответ уместным. Ни один реальный дефект от живых пользователей так не ловился: имя файла, которое агент проигнорировал, целые числа, опубликованные как 2.0, таблица, сплющенная в абзацы, формулы, заменённые руками набранными числами, свод по чужому листу, отклонённый как циклическая ссылка, пустой документ вместо готового. Отдельная проблема: продукт проверял сам себя своим же кодом, запись и чтение файла шли одной библиотекой, и баг записи прятался при чтении. От меня требовался стенд, который выносит вердикт по самому файлу, а не по внутренним событиям агента.

## Что я сделал

- Собрал единый стенд с двумя дорожками вместо девяти разных сьют: один инструмент дергается по gRPC за миллисекунды, а весь ход проверяется через API целиком с моделью.
- Заставил стенд читать готовый файл независимыми библиотеками: python-docx, openpyxl, python-pptx, pypdf, Pillow. Так вердикт не зависит от тех же `*.inspect`-инструментов, которыми продукт сам себя проверяет.
- Описал словарь утверждений о свойствах файла вместо сравнения байтов, например «в таблице не меньше N строк» или «итог это формула». Ход модели невоспроизводим дословно, а свойство результата переживает повторный прогон.
- Добавил исход VACUOUS: каждое утверждение проверяется ещё и на входном файле, и если ответ совпадает, кейс помечается как ничего не измеривший, а не как pass.
- Добавил режим повторов --repeats N и исход unstable для кейсов, где повторные прогоны расходятся вердиктом. Пробросил номер попытки в ключ идемпотентности и id беседы, иначе повторные вызовы читали историю прошлой попытки.
- Собрал корпус из 62 реальных русских документов с manifest.json: источник, sha256, размер. Провенанс проверяется тестом, а не обещанием в документации.
- Настроил оракулы так, чтобы каждый отклонял артефакт, который предыдущая версия проверки засчитывала как pass: склейку вместо соединения по ключу, выгрузку не того листа, презентацию с адресом из шаблона.

## Что было сложно

Проверка формул итога поначалу читала только кэшированные значения ячейки. Книга уезжала пользователю с итогом вида `SUM(Бюджет!#REF!:#REF!)`: колонку, которую суммировал свод, удалили, а стенд называл файл чистым. Причина в том, что ни одна книга, которую делает продукт, не имеет кэша значений, Excel её не открывал, и для каждой формулы читалось None. Я переделал проверку так, чтобы читались обе стороны: кэшированное `#VALUE!` означает, что формула выполнилась и упала, а токен `#REF!` в тексте формулы значит, что адрес потерян.

Первый живой прогон смоук-набора из шестнадцати кейсов дал шесть настоящих продуктовых падений и десять ложных. Пять кейсов ждали маршрут навыка, который в этом сценарии не исполняется, три требовали SVG без указания формата в просьбе, один требовал 150 символов там, где верный ответ укладывается в 104. Я переписал эти кейсы: подгонять продукт под них значит получить зелёную приёмку и красный прод.

Синтетические файлы не несли реальных дефектов: объединённых ячеек, листа на 16 000 колонок, формулы-самопроверки под подписью бухгалтера. Я собрал корпус из открытых публикаций ведомств и вузов, но часть документов несла чужие персональные данные: реестры с ФИО и ИНН, паспортные данные в анкетах. Такие файлы я вынес из репозитория совсем и завёл отдельный lock-файл с идентификаторами и хэшами, а тест отвергает файл с разъехавшимся хэшем.

## Результат

Стенд покрыл 107 кейсов на 16 семейств форматов, корпус вышел на 62 реальных русских документа с проверяемым провенансом, покрыт 81 навык из 86. Стендом найдены и починены реальные дефекты продукта: свод по чужому листу, отклонённый как циклическая ссылка, нередактируемая смета и потерянная ссылка в формуле итога. Сравнимого замера «до» нет: прежняя приёмка мерила другой предмет, и цифр вида «качество выросло на N%» в этом кейсе нет.

## Моя роль

Приёмочный контур целиком, от стенда и раннера до корпуса и оракулов, построил я один. За период с 11 по 28 августа 2026 я был третьим по числу коммитов из пяти контрибьюторов в основном репозитории. Архитектуру продукта, оркестратор и контракты protobuf делала команда и мне досталась готовой, а конвенции compose и CI были платформенными.
