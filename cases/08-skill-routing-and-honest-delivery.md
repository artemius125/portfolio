...
Кейс из работы в maracuya ai, стартапе с платформой ИИ-агентов для документов. Я работал
там LLM-инженером с июня по сентябрь 2026 года. Занимался двумя задачами: агент должен был
попадать в нужный навык из каталога в сотню с лишним инструментов и не выдавать
невыполненную работу за сделанную.
...
Платформа принимала произвольную просьбу человека с приложенным файлом и сама подбирала
навык из каталога в сотню с лишним инструментов. Точность проверял словарный роутер по
ключевым словам и показывал 57%, а основатель компании требовал не меньше 90%. Вторая проблема была
серьёзнее: продукт умел ошибаться и отказываться, но не всегда говорил об этом человеку. В
одном прогоне продукт получил отчёт о ремонте дорог вместо нужных данных, всё равно сделал
справку о доходах, пересказал в ней чужой отчёт и написал, что всё сходится. Каталог
навыков менялся без правки кода. В проде маршрутизацию вела слабая по тарифу модель, потому
что сильная модель была занята содержательной работой.

## Что я сделал

- Собрал каскадный роутер из двух вызовов модели: первый снимает отсылки к разговору и определяет категорию результата, второй выбирает навык из карточек этой категории. Так проще искать нужный навык в живом каталоге.
- Перевёл форсированные схемы роутера с вложенных объектов на плоский список полей для guided decoding по JSON Schema, потому что модель увереннее выбирает поле из плоской схемы.
- Оставил кейвордный роутер только фолбэком на случай, когда модельный классификатор недоступен, и убрал его как основной режим выбора вместе с обслуживающими ветками кода.
- Собрал сюиту из 191 промпта для проверки маршрутизации и подключил её как гейт в CI с порогом точности 0.9.
- Добавил проверки перед публикацией документа в docx: пакет файлов взвешивается до и после правки, диапазон с таблицами не режется без явного запроса пользователя.
- Закрыл кодом и тестом создание документов о человеке, которого нет в тексте источника, для справок и других документов, удостоверяющих личные данные.
- Построил цепочку проходов для длинного редактирования текста с ревьюером между проходами и отдельным шагом починки.

## Что было сложно

В реестре агентов было тридцать четыре агента, которые ничего не делали: принимали задачу,
писали две отладочные строки и возвращали текст о том, что задача принята. Их держали в
реестре, потому что четыре места в коде потом отфильтровывали их обратно. Я убрал агентов
вместе с фильтрами. Теперь любое удалённое имя агента получает честную ошибку «неизвестный
саб-агент» вместо тихого согласия.

Отдельная сложность: один кейс на навык не давал сигнала, только 0% или 100% в зависимости
от того, прошёл он или нет. Я перестроил гейт так, чтобы он судил общую точность по всей
сюите, но печатал разбивку по каждому навыку и пары путаницы: что роутер выбрал вместо
нужного. Навыкам с одним кейсом я добавил второй, сформулированный по-другому, чтобы
прохождение теста не означало, что роутер зацепился за одну случайную деталь.

Восстановление docx из PDF шло по текстовому слою, а не по фреймам конвертера, потому что
фреймы дают документ, который выглядит похожим, но не редактируется. Когда текстовый слой
был нечитаем, включался фолбэк на LibreOffice. Сравнение документов тоже подвело:
`docx.compare` видел только абзацы текста и пропускал ячейки таблиц, где обычно живёт смысл
сметы. Нашёл и второй баг того же класса: колонка с формулами читалась как колонка чисел, и
правка портила расчёт, оставляя внешне правильные цифры.

## Результат

Точность выбора навыка выросла с 57% до 90%: 86 из 150 промптов у словарного роутера против
403 из 450 у модельного каскада на трёх повторах той же сюиты. Сюита выросла до 191 промпта
в шести классах, проходит за минуту и возвращает ненулевой код, если точность падает ниже
0.9. Реестр агентов сократился с 41 записи до 7 настоящих. Продуктовых метрик, таких как
число сделанных документов или доля довольных ходов, компания не измеряла.

## Моя роль

Я сделал роутер и его схемы, гейт сюиты маршрутизации, обе проверки на публикации docx,
правило про человека, которого нет в источнике, цепочку длинного редактирования и перевод
оговорок на русский язык. По коммитам я был третьим контрибьютором из пяти за период кейса.
Каскадную схему выбора навыка и порог 90% определил не я: первое выработала команда,
второе поставил заказчик. Архитектура платформы, включая оркестратор и контракты protobuf,
существовала до меня.
