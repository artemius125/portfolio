...
Я сделал этот проект с января по июнь 2026 года в Клерк.Ру. Это бухгалтерский портал с около 7 млн визитов в месяц, я работал там единственным AI-разработчиком. Бот отвечает на обращения в открытых линиях Битрикс24 текстом статьи из базы знаний, собранной из реальных переписок операторов.
...
Операторы поддержки получали много однотипных обращений в открытых линиях Битрикс24. Доступ к подписке, вход в кабинет, документы об обучении, оплаты и возвраты повторялись каждый день. Ответы на них были похожи, но каждый раз их писал человек вручную. Компания поставила жёсткое ограничение: бот не сочиняет ответ, а использует текст статьи базы знаний дословно. Модель могла маршрутизировать обращение и подбирать статью, но не могла сочинять текст, который увидит клиент. Часть категорий, жалобы, оплаты и возвраты, удаление аккаунта, оставались только за человеком.

## Что я сделал

- Исследовал данные обращений и построил офлайн-пайплайн базы знаний: выгрузка истории из Битрикс24, маскирование персональных данных, отбор отвеченных диалогов, пакетная LLM-разметка с лимитом бюджета, канонизация в статьи, эмбеддинги.
- Построил care-ядро маршрутизации на FastAPI: классификация категории обращения, поиск статьи по эмбеддингам через numpy, LLM-сверка применимости, ответ текстом статьи дословно.
- Реализовал многоуровневый guard перед автоответом: запретные категории, порог косинусной близости, правило двух неудач подряд, проверка категории найденной статьи, отдельный гард летучих фактов вроде цены или промокода.
- Сделал отдельный движок деревьев решений для диагностических сценариев и механизм пошагового прохождения линейных статей.
- Добавил LLM-судью: он закрывает неактивные диалоги и выносит вердикт решено, эскалировано или не решено, по тем же критериям, по которым размечались диалоги операторов.
- Построил админку на Jinja2: редактирование базы знаний с переиндексацией, тестовый чат со стримом, диалоги, аналитика.
- Сделал внешний read-only JSON API базы знаний со статическим bearer-токеном и fail-closed конфигурацией, настроил тесты на pytest с aiosqlite и деплой через Docker и GitLab CI.

## Что было сложно

Калибровка порога уверенности на held-out наборе диалогов показала предел подхода. Целевая точность из плана не достигалась ни при одном пороге, метрика выходила на плато и деградировала на единичных сессиях. Расширение базы знаний почти не двигало цифры: узким местом было качество сопоставления эмбеддингами и покрытие базы относительно формулировок клиентов. Порог 0.78 в коде это компромисс, целевую точность он не обеспечивает; конкретные метрики и размеченный набор остаются внутренними данными компании.

Каталог исследовательских данных был исключён из Docker-образа, там лежали сотни мегабайт клиентских выгрузок. Модуль retrieval в рантайме импортировался именно оттуда через подмену пути. Локально всё работало, а контейнер падал на импорте ещё до старта веб-сервера. Я перенёс модуль в сервис, а в исследовательском пайплайне оставил тонкую обёртку.

До правки сообщение «привет» уходило в retrieval и не находило статью. Клиент получал заглушку эскалации на обычное приветствие. Я добавил классификатор нецелевых сообщений перед retrieval, теперь такие реплики получают короткий ответ. Промпт этого шага прямо запрещает отвечать по существу продуктовых вопросов.

## Результат

К моменту завершения проекта офлайн-пайплайн базы знаний, care-ядро, движок деревьев решений, судья, админка и внешний API были готовы. Выгрузка истории из Битрикс24 для построения базы знаний работала. Канал открытых линий Битрикс24 на момент разбора не был запущен. Продуктовых метрик на реальном трафике компания не собирала, потому что пилот ещё не стартовал.

## Моя роль

Я исследовал данные обращений и построил пайплайн базы знаний, care-ядро, движок деревьев решений, судью, админку, внешний API, схему БД и тесты. Часть кода унаследована от предыдущего этапа: оттуда взяты структуры дерева решений и каркас админки. Требования, границы автоматизации и критерии оценки диалогов определяла компания, я фиксировал эти решения в коде.
