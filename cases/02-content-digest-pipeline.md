...
Сделал в Клерк.Ру, бухгалтерском портале с около 7 млн визитов в месяц, с января
по июнь 2026 года. Я работал там единственным AI-разработчиком. Сервис собирает
материалы для дайджеста из трёх источников, генерирует текст через LLM и
публикует готовый HTML по расписанию.

## Коротко

- **Задача:** редакция портала собирала ежедневный и еженедельный дайджест вручную. Нужно автоматизировать сборку по расписанию и дать редактору панель управления.
- **Решение:** сервис на FastAPI собирает материалы из трёх источников, генерирует дайджест через LLM в строгом JSON, экспортирует HTML под CMS и публикует файл в облако. Промпты, расписание и история запусков в PostgreSQL, у редактора панель с живым логом.
- **Стек:** Python 3.11, FastAPI, Jinja2, SSE, PostgreSQL, SQLAlchemy 2.x async, Alembic, Docker, GitLab CI, Helm, Loki.
- **Результат:** пайплайн из четырёх шагов работает по расписанию и вручную за любую дату. В панели учёт токенов с оценкой в рублях, ретраи, защита от параллельных запусков. Метрику «сколько времени сэкономлено» заказчик не собирал.
- **Моя роль:** перенос shell-прототипа в сервис целиком: FastAPI, схема БД, планировщик, панель редактора, стриминг логов, учёт стоимости, контейнеризация и деплой.

## Что за задача

Редакция портала выпускала два регулярных дайджеста: ежедневный и еженедельный.
Каждый выпуск собирали руками. Редактор проходил по разделам, выбирал материалы,
переписывал аннотации и вручную вёрстал HTML под CMS. Первая версия автоматизации
была shell-обёрткой над CLI-скриптами, с конфигурацией в JSON-файле и промптами
в markdown-файлах на диске. При переезде в кластер такое файловое состояние не
пережило бы пересоздание пода, а с ним потерялись бы правки редактора. Нужен был
сервис с расписанием, панелью для редактора и устойчивостью к «тонким» дням,
когда материалов за сутки не хватает.

## Что я сделал

- Перенёс оркестрацию из shell-скриптов в сервис на FastAPI с планировщиком внутри процесса, без прав на кластерный шедулер.
- Оставил сборщики источников (лента материалов, клубная площадка, форум) отдельными CLI-процессами, оркестратор запускает их как дочерние процессы с таймаутами.
- Настроил генерацию дайджеста через LLM со строгим JSON-контрактом: валидация полей, проверка ссылок, три попытки с паузой.
- Сделал экспортёр HTML под требования CMS и публикацию файла в облачное хранилище через REST API.
- Перевёл конфигурацию и промпты из файлов на диске в PostgreSQL, чтобы правки редактора переживали пересоздание пода.
- Построил панель редактора на Jinja2: ручной запуск за любую дату, живой лог через SSE, история запусков, учёт стоимости токенов.
- Настроил контейнеризацию и деплой: Docker, GitLab CI, Helm для dev и prod, логи в Loki.

## Что было сложно

Самой дорогой ошибкой был дедлок на блокирующем чтении stdout. Живой лог читал вывод
дочернего процесса построчно, а проверка таймаута срабатывала только после получения
строки. Процесс, зависший на сетевом запросе до первого вывода, не давал ни строки, и
глобальный лок запусков оставался занят навсегда. Внешне это выглядело как два подряд
пустых запуска, сам зависший процесс в историю не попадал. Починил отдельным
потоком-читателем с очередью и таймаутом через queue.get, плюс гарантированным
terminate и kill в finally.

Вторая проблема: асинхронная БД падала при вызове из синхронного потока. Стриминговый
ответ выполнялся в threadpool, а общий engine был привязан к главному event loop.
asyncio.run поднимал новый loop, и asyncpg падал с ошибкой про future от чужого loop.
Решением стал одноразовый engine с NullPool внутри нового loop.

Третья группа проблем: гонки при двойном запуске и нехватка материалов за конкретный
день. Тик планировщика мог наложиться на ручной запуск редактора, поэтому ввёл
неблокирующий лок: второй запуск сразу получает понятное сообщение вместо гонки за
файлы. Для «тонких» дней сделал добор материалов из предыдущих дней с дедупликацией по
идентификатору или URL, а форум без API разобрал регулярными выражениями.

## Результат

Пайплайн из четырёх шагов (сбор, генерация, экспорт HTML, публикация) работает и по
расписанию, и вручную за произвольную дату. Расписание по умолчанию: ежедневный выпуск
в рабочие дни и еженедельный в конце недели, московская зона. В панели редактора есть
ручной запуск с живым логом, история запусков с токенами, учёт стоимости в рублях,
редактирование расписания и промптов без релиза. Метрику «сколько времени сэкономлено
редакции» компания не собирала.

## Моя роль

Я перенёс рабочий, но файловый и shell-ориентированный прототип в сервис целиком:
FastAPI, схему БД, планировщик, панель редактора, стриминг логов, учёт стоимости,
контейнеризацию и деплой. Доменные сборщики и экспортёр HTML портировал почти без
изменений, это было чужое накопленное знание об источниках. Промпты принадлежали
редакции, а конвенции кластера и CI/CD задавала команда компании.
